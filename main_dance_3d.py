...
def main(opt):
    ...
    net_pred = AttModel.AttModelResNetVel(in_features=in_features, kernel_size=kernel_size, d_model=d_model,
                                 num_stage=opt.num_stage, dct_n=opt.dct_n, device=opt.device)
    net_pred.to(device=opt.device)

    #optimizer = optim.Adam(filter(lambda x: x.requires_grad, net_pred.parameters()), lr=opt.lr_now)
    optimizer = model_utils.NoamOpt(128, 1.3, 4000,
        torch.optim.Adam(net_pred.parameters(), lr=0, betas=(0.9, 0.98), eps=1e-9))
...
def run_model(net_pred, optimizer=None, is_train=0, data_loader=None, epo=1, opt=None):
    ...
    st = time.time()
    for i, (p3d_1, p3d_2) in enumerate(data_loader):
        batch_size, seq_n, dim = p3d_1.shape
        # when only one sample in this batch
        if batch_size == 1 and is_train == 0:
            continue
        n += batch_size
        bt = time.time()
        p3d_1 = p3d_1.float().to(device=opt.device) # (32, 60, 75)
        p3d_2 = p3d_2.float().to(device=opt.device)
        p3d_1_sup = p3d_1.clone()[:, -out_n - seq_in:].reshape([-1, seq_in + out_n, dim // 3, 3])
        p3d_1_src = p3d_1.clone()
        p3d_2_sup = p3d_2.clone()[:, -out_n - seq_in:].reshape([-1, seq_in + out_n, dim // 3, 3])
        p3d_2_src = p3d_2.clone()

        p3d_1_out_all = net_pred(p3d_1_src, p3d_2_src, input_n=in_n, output_n=out_n, itera=itera)
        p3d_1_out = p3d_1_out_all[:, seq_in:]
        p3d_1_out = p3d_1_out.reshape([-1, out_n, 25, 3])

        p3d_1_out_all = p3d_1_out_all.reshape([batch_size, seq_in + out_n, itera, dim // 3, 3])

        # 2d joint loss:
        grad_norm = 0
        if is_train == 0:
            loss_p3d = torch.mean(torch.norm(p3d_1_out_all - p3d_1_sup, dim=3))
            loss_all = loss_p3d
            optimizer.zero_grad()
            loss_all.backward()
        ...
        if is_train <= 1:  # if is validation or train simply output the overall mean error
            mpjpe_p3d = torch.mean(torch.norm(p3d_1[:, in_n:in_n + out_n] - p3d_1_out, dim=3))
            m_p3d += mpjpe_p3d.cpu().data.numpy() * batch_size
        else:
            mpjpe_p3d = torch.sum(torch.mean(torch.norm(p3d_1[:, in_n:] - p3d_1_out, dim=3), dim=2), dim=0)
