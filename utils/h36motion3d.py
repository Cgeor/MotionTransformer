...
class Datasets(Dataset):
    ...
    def __init__(self, opt, actions=None, split=0, itera=1):
        """
        :param path_to_data:
        :param path_to_data:
        :param actions:
        ...
        :param sample_rate:
        """
        self.path_to_data = "/home/costa/data/h3.6m/dataset"
        self.split = split
            ...
            for action_idx in np.arange(len(acts)):
                ...
                if self.split <= 1:
                    for subact in [1, 2]:  # subactions
                        ...
                        the_sequence = torch.from_numpy(the_sequence).float().to(device=opt.device)
                    ...
                    the_seq1 = torch.from_numpy(the_sequence1).float().to(device=opt.device)
                    the_seq1[:, 0:6] = 0
                    ...
                    the_seq2 = torch.from_numpy(the_sequence2).float().to(device=opt.device)
                    the_seq2[:, 0:6] = 0
