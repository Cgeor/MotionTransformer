...
def fkl_torch(angles, parent, offset, rotInd, expmapInd):
    ...
    if torch.cuda.is_available():
        p3d = Variable(torch.from_numpy(offset)).float().to(device='cuda').unsqueeze(0).repeat(n, 1, 1)
    else:
...
def main():
    ...
    if torch.cuda.is_available():
        exp1 = Variable(torch.from_numpy(np.vstack((expmap_pred, expmap_gt))).float()).to(device='cuda')
    else:
