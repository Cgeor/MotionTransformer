...
def load_data_cmu_3d(path_to_dataset, actions, input_n, output_n, data_std=0, data_mean=0, is_test=False):
    ...
    for action_idx in np.arange(nactions):
        ...
        for examp_index in np.arange(count):
            ...
            action_sequence = readCSVasFloat(filename)
            n, d = action_sequence.shape
            exptmps = torch.from_numpy(action_sequence).float().to(device=device_type())
...
def rotmat2euler_torch(R):
    ...
    if len(idx_spec2) > 0:
        R_spec2 = R[idx_spec2, :, :]
        eul_spec2 = torch.zeros(len(idx_spec2), 3).float().to(device=device_type())
        eul_spec2[:, 2] = 0
        eul_spec2[:, 1] = np.pi / 2
    ...
    if len(idx_remain) > 0:
        ...
        eul_remain = torch.zeros(len(idx_remain), 3).float().to(device=device_type())
...
def rotmat2quat_torch(R):
    ...
    q = torch.zeros(R.shape[0], 4).float().to(device=device_type())
...
def expmap2rotmat_torch(r):
    ...
    n = r1.data.shape[0]
    R = torch.eye(3, 3).repeat(n, 1, 1).float().to(device=device_type()) + torch.mul(
