...
def main(opt):
    ...
    net_pred = AttModel.AttModelResNetVel(in_features=opt.in_features, kernel_size=opt.kernel_size, d_model=opt.d_model,
                                  num_stage=opt.num_stage, dct_n=opt.dct_n, device=opt.device)
    net_pred.to(device=opt.device)

    #model_path_len = '/home/costa/src/Transformer/checkpoint/trans_N6_in50_out10_ks10_dctn20/ckpt_best.pth.tar'
    model_path_len = '/home/costa/Desktop/ckpt_best.pth.tar'
    print(">>> loading ckpt len from '{}'".format(model_path_len))
    ckpt = torch.load(model_path_len, map_location=torch.device(opt.device))
...
def run_model(net_pred, optimizer=None, is_train=0, data_loader=None, epo=1, opt=None):
    ...
    for i, (p3d_h36) in enumerate(data_loader):
        ...
        n += batch_size
        bt = time.time()
        p3d_h36 = p3d_h36.float().to(device=opt.device)
